...
particle.  `cascade_decode.py` reads it once per group (one CTA holds all
N×G query rows).  Measured honestly: a **wash at 4k** — at that size the
kernel is bound by scattered-read latency, not traffic, so reading less
doesn't help — but 1.9×/3×/4.2× over stock at 8k/16k/32k prefixes.  It is
merged as tested groundwork, not wired into the hot path: dispatch cannot
branch inside a captured graph, so enabling it needs dual-graph capture
keyed on prompt-length regime.
...
| `cascade_decode.py` | (groundwork) grouped decode with shared-prefix reads | 1.9–4.2× at 8k–32k prefixes; wash at ≤4k |
