...
# The image clones eagle3-draft-experimental from origin (hybrid-models is
# currently a local-only fast-forward of that branch). The local working tree
# is then overlaid via add_local_dir / add_local_file so the runtime smcsd/
# and scripts/ match whatever is on disk — no push needed to iterate.
SMCSD_BRANCH = "eagle3-draft-experimental"
