"""Qwen3.6-27B baseline GSM8K (no speculative decoding) on the upstream branch.

Mirrors the runtime env from validate_upstream.py:
  - CUDA 13 base image, smcsd `upstream` branch + recursive submodules
  - torch==2.11.0 + sglang-kernel==0.4.2 installed at runtime
  - the same idempotent smcsd-side patches in case stale clones import smcsd
    (baseline mode itself only needs sglang)

Runs scripts/accuracy_test_gsm8k.py --mode baseline against Qwen/Qwen3.6-27B
on a single H200.

Usage:
    modal run modal_apps/run_qwen_baseline.py
    modal run modal_apps/run_qwen_baseline.py --num-questions 50 --max-new-tokens 1024
"""
...
def run_qwen_baseline(
    ...
    # Use the local workspace copy of the eval script (matches what's on the
    # user's branch even if origin/upstream is behind).
    shutil.copyfile(
        "/tmp/smcsd_local_accuracy_test_gsm8k.py",
        f"{SMCSD_DIR}/scripts/accuracy_test_gsm8k.py",
    )
    print("Patched scripts/accuracy_test_gsm8k.py from local workspace copy", flush=True)
    ...
    print(f"qwen baseline rc={rc.returncode}", flush=True)
    if rc.returncode != 0:
