    ...
    def free_group_slots(self, group_id: str) -> None:
        ...
        for slot in slots:
            ...
            if pool_idx != EMPTY_SLOT and alloc_len > 0:
                ...
                if req is not None:
                    # Hybrid: free the per-request mamba state slot before
                    # returning the req-pool slot. ReqToTokenPool.free only
                    # handles the req-pool side; on a HybridReqToTokenPool
                    # the mamba slot leaks unless explicitly returned.
                    if (
