...
import torch as _t
for NLrun in [2,8,16]:
    comp=cute.compile(draft,*mt,SP,NGEN,NLrun,H,Hkv,D,hid,I,GRP,DH,V,B,BLK,eps,scale,1.0/T)
    def run(): arr.zero_(); sen.zero_(); comp(*mt)
    for _ in range(3): run()
    _t.cuda.synchronize(); e0=_t.cuda.Event(True); e1=_t.cuda.Event(True); e0.record()
    for _ in range(20): run()
    e1.record(); _t.cuda.synchronize(); ms=e0.elapsed_time(e1)/20
    print(f"NL={NLrun}: {ms*1000:.0f} us/launch")
