    ...
    for t in cutlass.range(SP+NGEN-1):
        ...
        for L in cutlass.range(NL):
            ...
            while n<QW:
                acc=F32(0.0); k=lane
                while k<hid:
                    acc=acc+gh[0,k].to(F32)*inv*gg1[L,k].to(F32)*gWq[L,n,k].to(F32); k=k+32
                acc=wreduce(acc)
...
print(f"[M5a coalesced] pred={gen} ref={ref_g}  match {match}/{NGEN}")
...
print(f"[M5a timing] {ms*1000:.0f} us/launch, {ms*1000/(SP+NGEN-1):.0f} us/token  (was 61924 us/token naive)")
