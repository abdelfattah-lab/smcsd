"""M7a: draft(1B) + verify(8B) + IN-KERNEL tempered-logprob, all in ONE persistent kernel launch.
Extends M6c (m6c_cycle_mega.py): after the fused draft->verify forward, the verify's lm_head AND the
tempered target logprob per drafted position are now computed IN-KERNEL (reusing the M5b vocab-reduction:
parallel max -> grid-reduce, parallel sum-exp -> grid-reduce, logp=(logit/T)[tok]-logsumexp). This makes the
verify's actual OUTPUT (the logprobs the SMC reweight stage consumes) in-kernel, the prerequisite for M7b
(reweight/resample/bonus). The draft's sampled tokens still flow to the verify input IN-KERNEL (no host
round-trip) — the fusion SMC's static, no-rejection shape uniquely permits.
Validation: draft tokens 4/4 vs 1B Gumbel ref; verify top1 100% vs HF-8B; in-kernel logp matches the host
lm_head path (~5e-3) and HF-fp32 (~3e-2, within the bf16 floor). N=1 particle (M7b adds N>1 + resample).
...
def cycle_k(
    ...
    ghv,gQv,gKv,gVv,gAv,gRv,gActv,gVlog,gVlp,gRed,
    # ---- draft constexpr ----
    ...
    epsv:cutlass.Constexpr,scalev:cutlass.Constexpr,Vvoc:cutlass.Constexpr,
    # ---- launch constexpr ----
    ...
    # ===== VERIFY lm_head + TEMPERED TARGET LOGPROB, in-kernel -> gVlp[NGEN] =====
    # drafted position p (p in 0..NGEN-1): token at index mpos=SP-1+p predicts gVtok[mpos+1].
    # logp[p] = (logit/T)[gVtok[mpos+1]] - logsumexp(logit/T)   over the 8B target logits.
    for p in cutlass.range(NGEN):
        mpos=SP-1+p
        # final 8B norm of ghv[mpos], staged into smem
        ss=F32(0.0); k=lane
        while k<hidv: xv=ghv[mpos,k].to(F32); ss=ss+xv*xv; k=k+32
        invf=cute.rsqrt(wreduce(ss)/F32(hidv)+F32(epsv))
        i=tx
        while i<hidv: sY[i]=ghv[mpos,i].to(F32)*invf*gnormv[i].to(F32); i=i+BLK
        cute.arch.barrier()
        # lm_head GEMV (warp per output) -> gVlog[p,n]
        n=wid
        while n<Vvoc:
            acc=F32(0.0); k=lane
            while k<hidv: acc=acc+sY[k]*gLMv[n,k].to(F32); k=k+32
            acc=wreduce(acc)
            if lane==0: gVlog[p,n]=acc
            n=n+NW
        ls=I32(1)-ls; gbar(arr,sen,ls,B,tx)
        # pass 1: max over vocab of (logit/T)
        lmax=F32(-1.0e30); v=gtid
        while v<Vvoc:
            z=gVlog[p,v]*F32(invT)
            if z>lmax: lmax=z
            v=v+GT
        o=16
        while o>0:
        ...
        ls=I32(1)-ls; gbar(arr,sen,ls,B,tx)
        gmax=gRed[0]
        # pass 2: sum exp(logit/T - max)
        lsum=F32(0.0); v=gtid
        while v<Vvoc:
            lsum=lsum+cute.exp(gVlog[p,v]*F32(invT)-gmax)
            v=v+GT
        lsum=wreduce(lsum)
        ...
        if gtid==0:
            ...
            tokn=gVtok[mpos+1]
            gVlp[p]=gVlog[p,tokn]*F32(invT)-lse
        ls=I32(1)-ls; gbar(arr,sen,ls,B,tx)
...
def cycle(gTok,gVtok,gPred,gU,garr,gsen,
          ...
          Sv:cutlass.Constexpr,NLv:cutlass.Constexpr,Hv:cutlass.Constexpr,Hkvv:cutlass.Constexpr,Dv:cutlass.Constexpr,
          hidv:cutlass.Constexpr,Iv:cutlass.Constexpr,GRPv:cutlass.Constexpr,DHv:cutlass.Constexpr,
          epsv:cutlass.Constexpr,scalev:cutlass.Constexpr,Vvoc:cutlass.Constexpr,B:cutlass.Constexpr,BLK:cutlass.Constexpr):
    cycle_k(gTok,gVtok,gPred,gU,garr,gsen,
            ...
            Sv,NLv,Hv,Hkvv,Dv,hidv,Iv,GRPv,DHv,epsv,scalev,Vvoc,
...
normv=mv.norm.weight; LMv=vm.lm_head.weight
Vlog=mk.f32(NGEN,Vvoc); Vlp=mk.f32(NGEN,1).reshape(NGEN).contiguous(); Red=mk.f32(4,1).reshape(4).contiguous()
...
args=[Tok,Vtok,Pred,U,arr,sen,
      ...
      ghv,Qv,Kv,Vv,Av,Rv,Actv,Vlog,Vlp,Red]
mt=[from_dlpack(x) for x in args]
cc=(SP,NGEN,NLd,Hd,Hkvd,Dd,hidd,Id,GRPd,DHd,Vd,epsd,scaled,1.0/T,
    Sv,NLv,Hv,Hkvv,Dv,hidv,Iv,GRPv,DHv,epsv,scalev,Vvoc,B,BLK)
...
# ---- M7a: validate the IN-KERNEL tempered logprob gVlp[p] (T=0.7) for the NGEN drafted positions ----
# In-kernel logp[p] scores token gVtok[SP+p] at position SP-1+p. Compare to (i) the host lm_head path on the
# SAME fused hidden ghv (isolates the in-kernel reduction), and (ii) HF-8B fp32 (the bf16 floor).
ik=Vlp.float()                                            # in-kernel logprobs, T=0.7
host_draft=tlp(mylog, nx, T)[SP-1:SP-1+NGEN]              # host lm_head on fused ghv, same T
hf32_draft=tlp(ref8_32, nx, T)[SP-1:SP-1+NGEN]           # HF fp32 truth
ik_vs_host=(ik-host_draft).abs().max().item()
ik_vs_fp32=(ik-hf32_draft).abs().max().item()
print(f"[M7a in-kernel logp] gVlp={[round(x,3) for x in ik.tolist()]}")
print(f"[M7a in-kernel logp] vs host-lm_head(same ghv)={ik_vs_host:.3e}   vs HF-fp32={ik_vs_fp32:.3e}")

# PASS = draft tokens match, verify top1 100%, fused verify within bf16 floor, AND the in-kernel logprob
# reduction matches the host lm_head path tightly (reduction correctness) + within bf16 floor of fp32.
# ik_vs_host bound = f32-reduction-order agreement between two independent lm_head+logsumexp impls (~1e-2 on
# a logprob is "same computation"); ik_vs_fp32 bound = bf16 floor.
ok = (dmatch>=NGEN-1) and (v_agree>0.99) and (mine_vs_32 < max(2*floor, 3e-2)) \
     and (ik_vs_host < 1.5e-2) and (ik_vs_fp32 < max(2*floor, 3e-2))
