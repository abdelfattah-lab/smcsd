...
def draft_k(gTok, gPred, gU, gEmb, gWq,gWk,gWv,gWo,gg1,gg2,gWg,gWu,gWd, gcos,gsin, gnorm, gLM,
    ...
    QW=H*D; KW=Hkv*D
    # smem staging buffer: holds the (normed/weighted) GEMV input, computed once per block,
    # then read from smem by every warp-per-output GEMV instead of re-reading global per output.
    smem=utils.SmemAllocator()
    sY=smem.allocate_tensor(F32, cute.make_layout(I), byte_alignment=16)
    ls=I32(0)
    for t in cutlass.range(SP+NGEN-1):
        ...
        for L in cutlass.range(NL):
            ...
            # stage normed-weighted activation (gh*inv*g1) into smem
            i=tx
            while i<hid:
                sY[i]=gh[0,i].to(F32)*inv*gg1[L,i].to(F32); i=i+BLK
            cute.arch.barrier()
            # Stage1 Q (warp per output, reads staged input from smem)
            n=wid
            while n<QW:
                acc=F32(0.0); k=lane
                while k<hid:
                    acc=acc+sY[k]*gWq[L,n,k].to(F32); k=k+32
                acc=wreduce(acc)
            ...
            while n<KW:
                ...
                n=n+NW
            ls=I32(1)-ls; gbar(arr,sen,ls,B,tx)
            # Stage2 attention (thread per head). NOTE: warp-per-head was measured 1.25x SLOWER
            # on a clean GPU (bench_iso.py) — attention is barrier-bound, not thread-bound, so the
            # extra per-key warp-reduce only adds critical-path latency. Kept thread-per-head.
            i=gtid
            while i<H:
                h=i; kvh=h//GRP
                ...
                i=i+B*BLK
            ls=I32(1)-ls; gbar(arr,sen,ls,B,tx)
            # Stage3 O + residual (warp per output) — stage attn output in smem
            i=tx
            while i<QW:
                sY[i]=gA[0,i].to(F32); i=i+BLK
            cute.arch.barrier()
            n=wid
            while n<hid:
                acc=F32(0.0); k=lane
            ...
            # stage normed-weighted res1 (gR*inv2*g2) into smem
            i=tx
            while i<hid:
                sY[i]=gR[0,i].to(BF).to(F32)*inv2*gg2[L,i].to(F32); i=i+BLK
            cute.arch.barrier()
            n=wid
            while n<I:
                ...
                n=n+NW
            ls=I32(1)-ls; gbar(arr,sen,ls,B,tx)
            # Stage5 down + residual (warp per output) — stage act in smem
            i=tx
            while i<I:
                sY[i]=gAct[0,i].to(F32); i=i+BLK
            cute.arch.barrier()
            n=wid
            while n<hid:
                acc=F32(0.0); k=lane
        ...
        if t+1>=SP:
            ...
            ii=t+1-SP
            # stage normed-weighted final hidden (gh*invf*gnorm) into smem
            i=tx
            while i<hid:
                sY[i]=gh[0,i].to(F32)*invf*gnorm[i].to(F32); i=i+BLK
            cute.arch.barrier()
            n=wid
            while n<V:
                acc=F32(0.0); k=lane
...
print(f"[M5d smem-stage] pred={gen} ref={ref_g}  match {match}/{NGEN}")
...
utpt=ms*1000/(SP+NGEN-1)
print(f"[M5d timing] {ms*1000:.0f} us/launch, {utpt:.0f} us/token  (M5 baseline 2761 us/token; naive 61924)")
...
print(f"[summary] M5d megakernel {utpt:.0f} us/tok vs HF {hf_ms*1000:.0f} us/tok (M5 was 2761; naive 61924)")
