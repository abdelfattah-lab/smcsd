        ...
        if model_runner.spec_algorithm.is_speculative():
            if self.model_runner.is_draft_worker:
                if self.model_runner.spec_algorithm.is_smc():
                    pass  # SMC draft captures in DECODE mode (AR draft loop)
                elif self.model_runner.spec_algorithm.is_dflash():
                    pass  # DFLASH draft workers reuse this runner for TARGET_VERIFY mode
                else:
                    raise RuntimeError("This should not happen")
            is_smc_draft = self.model_runner.is_draft_worker and self.model_runner.spec_algorithm.is_smc()
            if not is_smc_draft:
