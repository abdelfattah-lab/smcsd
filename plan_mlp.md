...
This is what advisor Ryan Cotterell asked for; the message confirming
the interpretation was sent on 2026-04-28. **Wait for his confirmation
before writing training code if there's any ambiguity.**
...
data-parallel over batches). Standard PyTorch DDP via `torchrun
--nproc_per_node 4`. Don't use FSDP for the heads themselves — they're
...
Output: a JSON or CSV with `(layer_idx, mean_kl, median_kl,
weighted_kl)` and a printed table. Also useful: `target_minus_q_lp` at
