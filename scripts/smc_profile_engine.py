"""
Profile SMC runs through either the regular Engine API or the dedicated SMCEngine API.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Profile SMC with either offline Engine() or dedicated SMCEngine()."
    )
    parser.add_argument(
        "--engine-kind",
...
def main() -> None:
    ...
    if not args.profile_v2:
        if args.profile_steps and len(profile_stages) > 1:
            expected_artifacts = len(profile_stages)
        elif args.profile_steps and len(profile_stages) == 1:
            expected_artifacts = 1
    ...
    with build_engine(args) as engine:
        if args.engine_kind == "engine":
            ...
            write_json(run_dir / "server_info.json", dedicated_info)

            print("SERVER_INFO")
            print(json.dumps(dedicated_info, indent=2))
        ...
        artifacts = wait_for_artifacts(run_dir, min_count=expected_artifacts)
