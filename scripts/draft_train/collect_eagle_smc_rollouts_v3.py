#!/usr/bin/env python3
"""V3: Multi-seed-state SMC-native recurrent EAGLE rollout collector.

For each prompt this samples ``--seeds-per-prompt`` (S) i.i.d. seed tokens
x0 ~ p(.|prompt) and produces one v2-shaped record per seed. Each record has
its own (state, R, K) rollout independent of the others. This addresses
single-seed offline degeneracy (median ESS/R ~ 0.13 with S=1, R=8, K=2 even
on a strong warm-start) by exposing weighted-MLE training to multiple
next-token states per prompt, more closely matching SMC inference particle
states. With S=1 the output schema is identical to v2.

For SMC-SD proposal training. It matches the
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Number of i.i.d. x0 ~ p(.|prompt) seed states per prompt. With "
             "--scaffold-len > 0, S becomes the number of depth checkpoints "
             "spread across the scaffold. Default 1 = v2 behavior.",
    )
    p.add_argument(
        "--scaffold-len",
        type=int,
        default=0,
        help="If > 0, generate a target-sampled continuation of this length per "
             "prompt and collect S records at evenly-spaced depths along the "
             "scaffold (instead of S i.i.d. seeds at depth 1). This produces "
             "genuinely diverse states for SMC-native training when the target "
             "next-token distribution at the prompt boundary is too peaky.",
    )
    p.add_argument(
        "--gamma-train",
        type=int,
    ...
    S_seeds = max(int(args.seeds_per_prompt), 1)
    K = int(args.gamma_train)
    T = float(args.temperature)
    eps_T = max(T, 1e-6)
    scaffold_len = max(int(args.scaffold_len), 0)
    use_scaffold = scaffold_len > 0
    if use_scaffold and scaffold_len < S_seeds:
        # need at least S distinct positions in scaffold
        scaffold_len = S_seeds
        print(
            f"[v3] WARN: scaffold_len < S_seeds; bumping scaffold_len to {scaffold_len}",
            flush=True,
        )
    ...
    for ex_idx, row in enumerate(iter_jsonl(args.data)):
        ...
        # ---- 1. build per-checkpoint state list ----
        # Each "checkpoint" yields one v2-shaped record with its own
        # (context_ids, seed, target_h_3_seq, context_len).
        per_seed: List[Dict] = []  # list of dicts with keys: context_ids, seed, target_h_3, ctx_len
        if not use_scaffold:
            # multi-seed at depth 1 (v2 / v3 multi-seed mode)
            out0 = target(
            ...
            seed_all = torch.multinomial(
                seed_logp.exp(), num_samples=S_seeds, replacement=True
            ).squeeze(0)  # (S_seeds,)
            hs = out0.hidden_states
            target_h_3_seq_full = torch.cat(
                [hs[i] for i in layer_ids], dim=-1
            ).to(dtype)
            for seed_idx in range(S_seeds):
                per_seed.append(
                    dict(
                        context_ids=input_ids,  # (1, prompt_len)
                        seed=seed_all[seed_idx : seed_idx + 1],
                        target_h_3=target_h_3_seq_full,  # (1, prompt_len, 3H)
                        ctx_len=prompt_len,
                    )
                )
        else:
            # multi-depth scaffold mode
            with torch.no_grad():
                gen_out = target.generate(
                    input_ids=input_ids,
                    attention_mask=attn,
                    max_new_tokens=scaffold_len,
                    do_sample=True,
                    temperature=eps_T,
                    pad_token_id=tokenizer.pad_token_id,
                )
            scaffold_full = gen_out  # (1, prompt_len + L) where L <= scaffold_len
            actual_scaffold_len = scaffold_full.shape[1] - prompt_len
            if actual_scaffold_len < S_seeds:
                # generation hit EOS too early; fall back to v2-style at depth 0
                seed_logits = target(
                ...
                per_seed.append(dict(
                    context_ids=input_ids, seed=seed,
                    target_h_3=target_h_3_seq_full, ctx_len=prompt_len,
                ))
            else:
                # one forward pass on (prompt + scaffold) to get hidden states
                # at every position. We then slice per checkpoint.
                attn_full_scaf = torch.ones_like(scaffold_full)
                out_full = target(
                    input_ids=scaffold_full,
                    attention_mask=attn_full_scaf,
                    output_hidden_states=True,
                )
                hs_full = out_full.hidden_states
                target_h_3_full = torch.cat(
                    [hs_full[i] for i in layer_ids], dim=-1
                ).to(dtype)  # (1, prompt_len + L, 3H)
                # evenly spaced positions in [0, actual_scaffold_len - 1]
                if S_seeds == 1:
                    positions = [0]
                else:
                    positions = [
                        round((actual_scaffold_len - 1) * i / (S_seeds - 1))
                        for i in range(S_seeds)
                    ]
                for p in positions:
                    ctx_len = prompt_len + p
                    seed_p = scaffold_full[:, prompt_len + p : prompt_len + p + 1].squeeze(1)
                    per_seed.append(dict(
                        context_ids=scaffold_full[:, :ctx_len],  # (1, ctx_len)
                        seed=seed_p,
                        target_h_3=target_h_3_full[:, :ctx_len, :],  # (1, ctx_len, 3H)
                        ctx_len=ctx_len,
                    ))

        # iterate per-seed states (each yields one v2-shaped record)
        last_logw_var = 0.0
        last_ess = 0.0
        for seed_idx, st in enumerate(per_seed):
            ctx_ids = st["context_ids"]
            seed = st["seed"]
            target_h_3_seq = st["target_h_3"]
            ctx_len = st["ctx_len"]
            shifted_ids = ctx_ids.clone()
            if ctx_len >= 2:
                shifted_ids[:, :-1] = ctx_ids[:, 1:]
            shifted_ids[:, -1] = seed
            ...
            # ---- 4. recurrent unroll over R candidate paths, length K ----
            ...
            for t in range(K):
                ...
                h_next, logits_next = _eagle_prefill(
                    ...
                    dtype=dtype,
                    lengths=torch.full(
                        (R,), embed_context.shape[1], dtype=torch.long, device=device
                    ),
                )
                h_prev = h_next
            ...
            target_logits_path = (
                out2.logits[:, ctx_len : ctx_len + K, :].float()
            )
            ...
            topk_logps, topk_ids = torch.topk(
                target_logp_full, k=args.target_topk, dim=-1
            )
            ...
            rec = {
                ...
                "target_h_3": target_h_3_seq[0, -1].to(torch.float16).cpu(),
                "target_h_3_seq": target_h_3_seq[0].to(torch.float16).cpu(),
                "shifted_input_ids": shifted_ids[0].to(torch.int32).cpu(),
                "candidates": path_tokens.to(torch.int32).cpu(),
                "draft_logps_old": path_old_logp.to(torch.float32).cpu(),
                "target_logps": target_path_logp.to(torch.float32).cpu(),
                "target_topk_ids": topk_ids.to(torch.int32).cpu(),
                "target_topk_logps": topk_logps.to(torch.float32).cpu(),
                "gamma_train": K,
                "num_candidates": R,
                "temperature": T,
            }
            ...
            # cache last-record diagnostics for log line below
            with torch.no_grad():
                logw = (target_path_logp.sum(dim=1) - path_old_logp.sum(dim=1)).detach()
                last_logw_var = float(logw.var(unbiased=False).item())
                last_ess = float(F.softmax(logw, dim=0).pow(2).sum().reciprocal().item())
        ...
        if n_done % 25 == 0 or n_done == 1:
            ...
            rate = n_done / elapsed
            print(
                f"[v3 {n_done}/{args.num_prompts}] rate={rate:.2f}/s shards={shard_idx} "
                f"S={S_seeds} K={K} R={R} prompt_len={prompt_len} "
                f"last_record_path_logw_var={last_logw_var:.2f} last_ess={last_ess:.2f}/{R}",
                flush=True,
            )
    ...
    if rows:
        ...
        shard_idx += 1
    print(
        f"[v3] Done: {n_done} prompts (S={S_seeds} -> ~{n_done * S_seeds} records), "
        f"{shard_idx} shards -> {out_dir}",
        flush=True,
    )
