...
OUTFILE="${1:-sglang_1b_70b_triton_v2_${TIMESTAMP}.csv}"
...
for b in "${NUM_PROMPTS_LIST[@]}"; do
  ...
  if SGLANG_ENABLE_SPEC_V2=True python -m sglang.bench_offline_throughput \
      --model-path "$MODEL" \
    ...
    if [[ -z "$tps" ]]; then
      ...
      echo "standalone_triton_v2,0,1,ERROR,$b" >> "$OUTFILE"
    else
      echo "standalone_triton_v2,0,1,$tps,$b" >> "$OUTFILE"
      echo "  -> tps=$tps"
    fi
  else
